...
def test_bump_self_check_failure_aborts(tmp_path, monkeypatch):
    ...
    assert "Restore the bump's edits with:" in msg
    assert "restore it first with: git checkout scripts/check_version_consistency.py" in msg
...
def test_bump_self_check_failure_lock_only_advice(tmp_path, monkeypatch):
    """The lock-only gate fires for two classes it can't reliably tell apart —
    the check ran and reported drift (echoed above), or the check script was
    replaced and exited 0 without its OK verdict — so the advice must hedge
    both and never suggest checking out the bump's own files."""
    make_tree(tmp_path, version="0.5.2", lock_version="0.5.1")
    fake, _ = make_runner(self_check_ok=1)
    with pytest.raises(SystemExit) as exc:
        run_bump(tmp_path, monkeypatch, new_version="0.5.2", runner=fake)
...
def test_bump_gate_lock_only_replaced_script_advice(tmp_path, monkeypatch):
    """The lock-only gate's replaced-script class (exit 0 without the OK
    verdict, nothing echoed): the advice must lead with the restore-first
    branch, not 'fix the drift'."""
    make_tree(tmp_path, version="0.5.2", lock_version="0.5.1")
    ...
    with pytest.raises(SystemExit) as exc:
        run_bump(tmp_path, monkeypatch, new_version="0.5.2", runner=silent_pass)
    msg = str(exc.value.code)
    assert "restore it first with: git checkout scripts/check_version_consistency.py" in msg
    assert "git checkout pyproject.toml" not in msg
    assert "git checkout uv.lock" not in msg
