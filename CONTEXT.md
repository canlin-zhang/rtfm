...
**Web source**:
A Source backed by a URL; Refresh re-fetches and caches the pages locally. Typed by sync
method (re-fetch) and flavored by hosting family — see **Flavor**. Best-effort and
cooperative-only: it indexes whatever extractable text a page serves and cannot read
image-only or JavaScript-rendered sites (no OCR, no JS execution — the same boundary that
applies to image-only PDFs).
_Avoid_: url source, http source, remote source

**Flavor**:
For a Web source, the parsing strategy for one hosting family — how pages are discovered,
how a version maps to URLs, and how content is extracted. The Sync method is shared by all
flavors; only the parsing differs. Current flavors: **readthedocs** (sites generated by
ReadTheDocs, hosted on readthedocs.io or a custom domain). A Web source holds only its `url`;
the flavor trusts the URL as given and attempts the parse — failure is reported as per-source
index state, never a load-time shape gate.
_Avoid_: subtype, kind (ambiguous with per-file handling)
